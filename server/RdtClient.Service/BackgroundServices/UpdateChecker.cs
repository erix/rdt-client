...
public class UpdateChecker(ILogger<UpdateChecker> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                var gitHubReleases = await GitHubRequest<List<GitHubReleasesResponse>>("/repos/erix/rdt-client/tags?per_page=1", stoppingToken);
                ...
                var gitHubSecurityAdvisories = await GitHubRequest<List<GitHubSecurityAdvisoriesResponse>>("/repos/erix/rdt-client/security-advisories", stoppingToken);
